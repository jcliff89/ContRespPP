...
#' @param tau.int The initial value for the precision of the ANOVA model, tau.
#' @param a.t Hyperparameter alpha for gamma prior on the precision of the ANOVA model, tau.
#' @param b.t Hyperparameter beta for gamma prior on the precision of the ANOVA model, tau.
...
#' @export
gibbs.sampler <- function(X, Y, n.seen, beta.mean, tau.int, a.t, b.t,
  ...
  if(! is.numeric(tau.int)) { stop("tau.int must be a numeric type.") }
  if(! is.numeric(a.t)) { stop("a.t must be a numeric type.") }
  if(! is.numeric(b.t)) { stop("b.t must be a numeric type.") }
  ...
  for(o in 2:n.sim) {
    ...
    for(k in 2:b.sim){
      ...
      for(p in 1:(num.param - 1)) {
        ...
        param.var <- 1 / (post[(k - 1), (num.param + 1)] * prior.tab[p, 3] + prior.tab[p, 1])
        ...
        bottom.frac <- prior.tab[p, 3] + (prior.tab[p, 1] / post[(k - 1), (num.param + 1)])
        ...
        sum.diff <- sum(data.list[[p]][, 1] - data.list[[p]][, 2:ncol(data.list[[1]])] %*% param.it.mod)
        ##### Finally, we actually calculate the parameter estimate for the p-th model parameter
        param.hat <- (1 / bottom.frac) * sum.diff + ((prior.tab[p, 1] / post[(k - 1), (num.param + 1)]) / bottom.frac) * prior.tab[p, 2]
      ...
      param.var <- 1 / (post[(k - 1), (num.param + 1)] * prior.tab[num.param, 3] + prior.tab[num.param, 1])

      #calculate mean for parameter | data and other parameters
      bottom.frac <- prior.tab[num.param, 3] + (prior.tab[num.param,1] / post[(k-1), (num.param + 1)])
      param.it.ref <- as.matrix(param.it[-(prior.tab[num.param, 4]), 1])

      sum.diff <- sum(data.list[[num.param]][, 1] - data.list[[num.param]][, 2:ncol(data.list[[num.param]])] %*% param.it.ref)
      param.hat <- (1 / bottom.frac) * sum.diff + ((prior.tab[num.param, 1] / post[(k - 1), (num.param + 1)]) / bottom.frac) * prior.tab[num.param, 2]
      ...
      tau.sum <- sum((full.design[, 1] - full.design[, 2:ncol(full.design)] %*% param.it) ^ 2)
      b.val <- b.t + 0.5 * tau.sum
    ...
    ##### Remove Burn in from conditional posterior draws
